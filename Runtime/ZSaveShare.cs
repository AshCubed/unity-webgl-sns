...
namespace Zappar.Additional.SNS
{
    ...
    public class ZSaveNShare
    {
        ...
        [DllImport(PluginName)]
        private static extern void zappar_sns_jpg_snapshot(byte[] img, int size);
        [DllImport(PluginName)]
        private static extern void zappar_sns_png_snapshot(byte[] img, int size);
        [DllImport(PluginName)]
        ...
        private static void zappar_sns_jpg_snapshot(byte[] img, int size) { }
        private static void zappar_sns_png_snapshot(byte[] img, int size) { }
        ...
        public static IEnumerator TakeSnapshot(SnapshotCaptured onCapturedCallback = null)
        {
            ...
            zappar_sns_jpg_snapshot(img, img.Length);
        ...
        {
            if (onSaved != null)
                m_snsListener.OnSavedPrompt += onSaved;
            if (onShared != null)
                m_snsListener.OnSharedPrompt += onShared;
            if (onClosed != null)
                m_snsListener.OnPromptClosed += onClosed;
        }
